...
#     # Adjusted normalization for better contrast
#     texture_screen = cv2.normalize(color_image, dst=None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX).astype(np.uint8)
...
def software_trigger():
    ...
    with Harvester() as h:
        ...
        with h.create({'id_': device_id}) as ia:
            ...
            features.ExposureTime.value = optimal_exposure
            features.Gain.value = optimal_gain
